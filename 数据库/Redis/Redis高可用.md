...
- 如果是**脑裂**，会导致旧的master写入的数据无法同步到新的master。为了防止这种数据丢失，Redis 提供了 min-slaves-to-write 和 min-slaves-max-lag 参数。这两个参数可以设置最少需要多少个从节点在线，以及从节点的最大延迟时间。由于主节点无法再连接到任何从节点，或者延迟超过了设定值，主节点就会自动拒绝所有写请求。
```bash
# 设置主节点能进行数据同步的最少从节点数量
min-slaves-to-write 1 #集群模式下可以设置为quorum值，避免因为网络分区导致数据丢失
...
Redis 中的哨兵用于监控主从集群的运行状态，并在主节点故障时自动进行故障转移。哨兵其实是一个运行在特殊模式下的 Redis 进程，所以它也是一个节点。哨兵节点主要负责：定时监控、主客观下线、领导者选举和故障转移。
...
- 迁移分配：当集群需要扩容或缩容时，Redis 提供了 `cluster reshard` 命令，可以重新分配哈希槽。批量迁移槽内所有 key到目标节点，迁移过程中，源节点和目标节点都可以继续提供服务。
...
Redis Cluster 在扩容（添加节点并迁移槽位）和缩容（迁移槽位并移除节点）期间可以继续提供服务，整个过程是在线进行的，不会导致集群整体不可用，但需注意部分操作可能带来短暂的性能影响或个别命令的阻塞。